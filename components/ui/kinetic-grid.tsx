...
// ─── Constants ────────────────────────────────────────────────────────────────
// Tuned for section-scale use (not full-viewport heroes) — smaller cells and a
// tighter influence radius read better inside a ~500-900px-tall band.
...
// ─── Component ────────────────────────────────────────────────────────────────
// A section-scoped variant of the original full-viewport KineticGrid: it fills
// whatever box its own content gives it (no min-h-screen), sizes itself via
// ResizeObserver instead of window dimensions, tracks the pointer only while
// it's over this element, and never paints its own background — the canvas is
// fully transparent so it overlays an existing colored section instead of
// replacing it with a dark theme.
...
export default function KineticGrid({
  ...
  const draw = useCallback(
    (now: number) => {
      ...
      // Fully transparent — this overlays whatever background the section
      // already has instead of painting its own.
  ...
  // ── Setup: size to the element itself, not the viewport ─────────────────────
  // The rAF loop is declared as a plain function inside the effect (rather than
  // a self-referencing useCallback) so it can call itself without tripping the
  // "immutability" lint rule around stale closures.
  ...
  // ── Pointer tracking (scoped to this element, not the window) ───────────────
  ...
  return (
    ...
    >
      <canvas
        ...
        aria-hidden="true"
      />
      {children != null && <div className="relative z-10 w-full">{children}</div>}
    </Tag>
  );
}
