...
export function SiteHeader({ home = false }: { home?: boolean }) {
  ...
  return <>
    ...
    {menuOpen && <nav className="mobile-nav container" id="mobile-nav" aria-label="Navegación móvil">{[...links, { href: contactHref, label: 'Hablemos' }].map(link => <a key={link.label} href={link.href} onClick={() => setMenuOpen(false)}>{link.label}<ArrowUpRight size={18}/></a>)}</nav>}
  </>;
}
